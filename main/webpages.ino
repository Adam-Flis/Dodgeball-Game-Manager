...
function load() {
  ...
  // Set the source of the audio object to the URL of the ESP32 server
  //audio.src = 'http://192.168.4.1:80/alarm.wav';
  // Load the audio file
  //audio.load();
}
...
function updatePage() {
    ...
    if (this.readyState == 4 && this.status == 200) {
      ...
      if (side.violation && (Date.now() - lastAudio) >= 1000) { // Alarm every second if violation occurred 
        // Play the audio file
        //audio.play();
        lastAudio = Date.now();
      }      
